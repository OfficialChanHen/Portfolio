...
export default function Projects() {
        ...
        introTl
            ...
            .fromTo(".scroll-down",
                { y: -20, opacity: 0 },
                {
                    ...
                    onComplete: () => {
                        gsap.to(".scroll-down", {
                            y: 10, duration: 0.75, repeat: -1, yoyo: true, ease: "sine.inOut",
                        });
        ...
        gsap.utils.toArray<Element>(".fade-in").forEach((el) => {
            ...
            gsap.fromTo(el, { y: 30, opacity: 0 }, {
                ...
                scrollTrigger: {
                    ...
                    end: () => "bottom 50%", toggleActions: "play none none reverse"
                },
            });
        });
    ...
    function handleProjectClick() {
        ...
        if (!projectOpen) {
            ...
            // Snapshot overlay to exact card position
            ...
            // Fade out content first, then collapse
            gsap.to(".project-content", {
                opacity: 0,
                y: 20,
                duration: 0.2,
    ...
    function handleNextClick() {
        setCurrentIndex(prev => (prev + 1 >= images.length ? images.length - 1 : prev + 1));
    }
    ...
    return (
        <div className="relative w-screen flex flex-col justify-start items-center bg-secondary">
            ...
            {/* Project Overlay — sibling to sections, absolute inside scroller */}
            <div ref={overlayRef} className="hidden display-none">
                <img src="/TFT.png" alt="TFT expanded" className="absolute inset-0 w-full h-full object-cover" />
                {/* Dark gradient overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent" />
                
                {/* Content + Close — all inside max-width container */}
                <div className="absolute inset-0 flex flex-col justify-end">
                    <div className="relative w-full max-w-[1080px] h-full mx-auto px-10 pb-10 flex flex-col justify-end">
                        ...
                        <h2 className="project-content text-4xl md:text-6xl font-bold text-white mb-4">Teamfight Tactics</h2>
                        <p className="project-content text-white/70 max-w-xl mb-6">A description of the project goes here.</p>
                        <div className="project-content flex gap-3 flex-wrap mb-8">
                            <span className="px-3 py-1 bg-white/10 border border-white/20 rounded-full text-sm text-white">React</span>
                            <span className="px-3 py-1 bg-white/10 border border-white/20 rounded-full text-sm text-white">TypeScript</span>
                        </div>
            ...
            {/* Projects Section */}
            <section id="projects" className="relative w-screen max-w-[1080px] h-dvh flex flex-col justify-center items-center p-10">
                ...
                <div className="projects-carousel w-[clamp(300px,120vw,1400px)] h-full flex flex-row justify-center items-center gap-[clamp(8px,2vw,24px)]">
                    {/* Card — click target, stays in place */}
                    <div
                        ref={cardRef}
                        className="relative w-[clamp(300px,40vw,600px)] h-[clamp(300px,90%,800px)] flex flex-col justify-center items-center p-[clamp(8px,1vw,12px)] bg-gradient-to-br from-white/5 to-white/[0.02] border border-white/20 rounded-2xl cursor-pointer"
                        onClick={handleProjectClick}
                    >
                        <div className="relative w-full h-full rounded-2xl overflow-hidden">
                            <img src="/TFT.png" alt="TFT image" className="absolute bottom-0 h-full w-[160%] object-cover"/>
                        </div>
                        <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 p-[clamp(8px,1vw,12px)] px-[clamp(12px,2vw,20px)] bg-highlight rounded-full text-[clamp(0.875rem,2vw,1.5rem)] text-nowrap">
                            Teamfight Tactics
                        </div>
                    </div>
                </div>

                {/* Nav buttons — absolute so they don't affect carousel height */}
                <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 flex justify-center items-center gap-4 transition-opacity duration-300 ${projectOpen ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
                    <button onClick={handlePrevClick} className="flex items-center justify-center w-11 h-11 rounded-full bg-white/10 border border-white/20 text-white hover:bg-white/20 transition-all duration-200 cursor-pointer">
                        <ChevronLeft className="w-[clamp(16px,2vw,20px)] h-[clamp(16px,2vw,20px)]" />
                    </button>
                    <div className="flex items-center gap-2">
                        {images.map((_, i) => (
                            <button key={i} className={`h-2 md:h-3 rounded-full transition-all duration-300 cursor-pointer ${i === currentIndex ? 'w-8 md:w-15 bg-white' : 'w-2 md:w-3 bg-white/30'}`} />
                        ))}
                    </div>
                    <button onClick={handleNextClick} className="flex items-center justify-center w-11 h-11 rounded-full bg-white/10 border border-white/20 text-white hover:bg-white/20 transition-all duration-200 cursor-pointer">
